...
> 历史设计记录（非运行时资源）。当前 skill 已收敛为唯一的 XREAL Style；本文保留用于追溯此前的双主题架构讨论。

> 基于对两份历史模板与全部 references 的逐行对比分析(2026-07)。
...
**不建议**把两个模板合并成一个"大一统模板"——两种风格的美学规则互斥（克制的 3px 小圆角 vs 大圆角卡片化、衬线 vs 无衬线），强行合并会让类名前缀和条件分支爆炸，也违背"每份 deck 单文件交付"的定位。
...
- 图片规则收敛为媒体资产库选择与槽位适配:资产比例匹配时铺满,必须保留原始比例时使用 `fit-contain`
