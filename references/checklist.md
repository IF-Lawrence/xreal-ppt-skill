...
预检(生成前)
  ...
  □ S02 纵向时间线有统一列头；宽度为正文的 72%-82%，每个节点都包含可见且对齐的 dot、年份、同口径指标、阶段名和体验影响；贯穿轴按行连续、横线没有松散延伸到整页边缘
...
内容
  ...
  □ S07 每个 `.row-fill` 都以 `--value` 持久保存真实宽度；动效结束和 B 静态模式下仍可见，实际宽度与 `--value` 一致
  □ S23 四边 plot frame 完整，首末柱与边界至少 28px、没有贴边或裁切，最高数值留在 plot 内；每个 `.chart-value` 与对应柱体水平中心对齐，且不依赖水平 transform
  □ S24 横轴确实是时间或连续变量,没有把无顺序类别强行连线；四边 plot frame 完整，`.line-geometry` 左右至少 28px，首末点、描边和终值标签都没有越界
