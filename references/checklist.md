...
- S28 缺少 6-9 卡、恰好 1 个主卡、2-3 个中卡、至少 3 种面积或来源；卡片越界、重叠、媒体契约错误或面积与优先级无关
...
- 内容超出时,先删减重复文案或换 Sxx 版式,不要把正文、卡片说明或图表标签压到 10/11/12/13px；若用户要求同页且信息相互依赖，使用 S27 重排，不自动拆页。
...
- `S27 Dense Synthesis` 只用于三组相互依赖信息必须同页的情况；固定为总述带 + 3 个等高主面板 + 7-12 个条目。总述标签与结论共享同一左轴，主面板依靠统一中性底色、等高外框和留白建立层级，内部条目不重复堆叠分割线。正文 ≥16px，meta ≥14px，最多 1 个黑色焦点区和 1 张语义媒体；媒体只能作为焦点区的 `cover + darken` 背景，不做小缩略图。
- `S28 Priority Bento` 只用于一个明确主卖点、2-3 个次级技术点和若干支持信息；固定为 12×6 网格、6-9 卡、恰好 1 个主卡、2-3 个中卡且至少 3 种面积。每卡必须声明 `--col/--span/--row/--rows`，面积必须对应真实优先级，不得随机拼贴。
