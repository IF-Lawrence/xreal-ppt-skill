...
理性、专业、可信、科技、先锋。页面应当冷静、克制、有结构，禁止装饰堆叠和任何霓虹效果。
...
--grey-3: #667085;         /* 辅助文字 */
--text-helper: #6E6E6E;    /* 白底与 --grey-1 上均满足 AA 的辅助文字 */
...
- 所有可见文字按最终渲染表面检查 WCAG AA：普通文字至少 `4.5:1`，大字（≥24px，或 ≥18.66px 且字重 ≥600）至少 `3:1`。这里的“最终表面”包括继承色、文字 alpha / opacity、透明卡片与所有祖先背景；禁止只看 CSS token 名称就假定可读。
- 白/浅灰表面使用显式 `--text-primary / --text-secondary / --text-helper`；黑色、accent 与深色媒体表面显式使用反白角色。不要让深色卡片继承深色正文，也不要让浅色卡片继承白字。对比不足时优先改文字角色、表面色、蒙版、裁切或素材，不增加白色/半透明文字面板。
- 文字直接压在媒体上必须声明 `data-media-contrast="darken"`。模板默认 `--media-scrim-alpha:.56`；同时保证 `image brightness × (1 - 最浅中性黑蒙版 alpha) <= .44`，并再次检查每个文字角色的最终对比度。辅助文字与单位若因透明度不足，应提高反白透明度；不能只让标题可读。
...
- 最终交付只保留页面真实渲染使用的字体族、字重和字形对应的完整 OTF；实际使用多个字体族或字重时全部保留，不按语言机械打包整套字体。
- 禁止字体子集化、按字符裁剪、重编码或生成精简字体文件。减少包体只能通过删除未引用的完整字体文件实现，确保后续修改文字时字体仍完整可用。
...
- KPI / 图表展示级大数字的单位统一使用右上肩位：`°`、`%`、`ms`、`Hz`、`in` 等都采用 `vertical-align:text-top`，默认以 `--unit-mark-opacity:.62` 形成一致的中性对比，并通过 `white-space:nowrap` 避免与数字拆行；当单位位于压图文字或红色关键值上、`.62` 无法满足 AA 时，局部提高到不透明反白/语义色。文字单位使用 `--unit-mark-gap:.18em`；角度符号使用更紧的 `--unit-degree-gap:.03em`，保持附着但不粘连。`screen` 等多字符英文词单位使用 `.unit-word` 恢复正常字距与词距，禁止继承 KPI 数字的负 tracking。正文句子中的单位不使用 `.unit`，作为普通文本随正文基线。角度禁止放入 `<sub>`；只有科学指数及数学/化学语义使用真正的上标或下标。
...
- 原生 S23/S24 使用四边同色同粗的单一 1px `--chart-frame`；第一条和最后一条网格线不得与 frame 重叠形成双描边。S23 首末柱与边界至少留 28px；S24 的 SVG、端点和终值标签统一放入左右至少 28px 的 `.line-geometry`，横轴标签使用相同安全区，不能靠裁切掩盖越界。终值标签使用透明底，停在对应端点左上肩位，与点位横纵方向至少相隔 4px。
- 来源与说明注脚统一使用 `--footnote-size:max(11px,.62vw)`，左端对齐页面内容轴，底部统一停在 `--footnote-bottom-offset`；不使用 border、分割线、底色或额外 padding。该字号仅属于底部来源/说明，不得套用到图表标签、正文或卡片说明。
- 底部注脚和底栏默认不生成。只在必要来源、方法/样本口径、法务或风险免责声明，以及对当前结论有信息增量时保留；若标题区已说明“虚构/示意”，底部不得重复。产品名、口号、芯片名、功能列表与制作说明不能作为填空式页脚。
- `S23 Data Chart` 使用共同零基线比较 3-8 个类别 × 2-4 个同量纲系列；默认按原始 `data-value` 自动生成 5 级 nice scale，最高柱应合理占据绘图区，不复用示例纵轴。只有跨图共享量纲时才使用 fixed min/max。`S24 Line Chart` 只用于时间或连续变量,限制 1-3 条线。
...
- `S27 Dense Synthesis` 只在大量相互依赖信息必须同页呈现时使用。总述标签与结论共享同一左轴；三个并行主面板统一浅灰底、1px 中性外边界、四边等距 padding 和 8px 圆角，但列宽与组内行高按内容量分配。内部主要靠标题层级、列宽与留白组织，不为每个条目重复画 hairline，也不复制参考图的蓝色主色、UI 控件、彩色面板或大圆角。横向三项比较若难以区分，可用三个等宽、无边框、无阴影的浅中性子区域，并将每项文字紧凑聚合在顶部。全页最多一个黑色焦点区和一张语义媒体；焦点项必须有 `data-focus-reason` 且由内容语义决定，不能固定套在某个序号。媒体只作为该焦点区的 `cover + darken` 背景，不做小缩略图。编号列固定宽度并与后方文本共享左轴。
- `S28 Priority Bento` 通过卡片面积表达同一主题内的真实卖点优先级，不通过多色卡片制造层级。它保留 12×6 网格和统一视觉，但必须在 `left-focus`、`right-focus`、`panorama`、`center-focus` 四种登记剪影中按内容关系和媒体构图选择，不得随机换版或机械复用同一坐标。恰好一个主卡、1-4 个中卡和至少两个支持卡共享白/浅灰/近黑体系，每张卡独立使用 8px 小圆角、1px 中性边界、等距 padding 和无阴影表面；至少三种面积，最多一个红色语义点。媒体只用于语义证据，压图文字使用中性深色蒙版并直接反白，不叠白色面板。
...
- 产品标志的视觉层级服从页面语境。S12 manifesto 透明身份行中的产品标志只是身份落款，渲染宽度应为页面内容宽度的 10%-16%，标准使用 `min(14vw,220px)`；不得放大成第二主标题，也不在其后铺整块黑色通栏。
